...
- O noVNC local continua restrito ao gateway publicado localmente. Em produção, Chrome/Selenium/noVNC rodam no Docker do servidor da loja e são publicados por Tailscale Funnel em uma porta HTTPS separada do Apache existente. WebDriver e noVNC não são publicados diretamente; o Spring usa uma chave interna e o modal recebe apenas token HMAC temporário, revogado no cancelamento/conclusão e limitado à tentativa atual.
