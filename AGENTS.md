...
- O RedeASSO recebe somente o nome de usuário para identificar a tentativa. Usuário e senha da Área Central são digitados manualmente no Chrome gráfico isolado; a senha não passa pelo React, Worker, Spring ou gateway e não pode ser persistida, devolvida em JSON, registrada em log, trace, auditoria, `localStorage` ou `sessionStorage`.
- O CAPTCHA nunca será clicado, resolvido ou contornado por automação. O Chrome gráfico real é exibido por noVNC dentro de um modal do próprio RedeASSO, sem abrir nova guia; a pessoa digita as credenciais e confirma manualmente a caixa “Sou humano” na página real da Área Central.
- O noVNC local continua restrito ao gateway publicado localmente. Em produção, Chrome gráfico/noVNC rodam no Docker do servidor da loja e são publicados por Tailscale Funnel em uma porta HTTPS separada do Apache existente. Nem noVNC nem a porta DevTools do Chrome são publicados diretamente; o Spring usa uma chave interna e o modal recebe apenas token HMAC temporário, revogado no cancelamento/conclusão e limitado à tentativa atual. O gateway usa DevTools exclusivamente para abrir a página, ler o cookie jar e detectar o formulário de senha, nunca para preencher credenciais ou interagir com CAPTCHA.
...
- Exceção confirmada em 22/08/2026: o navegador assistido da Área Central roda no servidor Docker/Tailscale. API, Neon e SPA continuam independentes dele; se o servidor ou Funnel estiver desligado, apenas o login/consulta da Área Central fica indisponível.
...
- `.github/workflows/docker.yml` valida as imagens Docker em pull requests e branches fora da `main`. Em cada push na `main`, publica no GHCR as tags imutáveis por SHA e as tags operacionais `main` (API) e `area-central-browser-main` (navegador); o Watchtower do servidor consulta essas tags a cada cinco minutos.
...
- `docker-compose.area-central.yml` usa o gateway para desenvolvimento com Spring local. `docker-compose.area-central-browser.server.yml` inicia apenas Chrome gráfico/noVNC/gateway no servidor da loja; noVNC e a depuração remota do Chrome ficam internos, enquanto o gateway valida chave de serviço e token temporário. O Funnel principal já encaminha ao Apache, portanto o navegador usa a porta HTTPS 8443 do mesmo hostname. A API Render aponta para essa URL e só habilita a integração após os segredos pareados e URLs do Funnel serem configurados no painel.
